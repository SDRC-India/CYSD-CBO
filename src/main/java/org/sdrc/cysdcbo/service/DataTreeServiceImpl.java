...
public class DataTreeServiceImpl implements DataTreeService {
	...
	@Resource(name="putChoiceDetails")
	...
	public List<BubbleDataModel> getBubbleChartData(Integer sectorId,int areaId,int timeperiodId) {
		...
		if (((CollectUserModel) stateManager.getValue(Constants.USER_PRINCIPAL))
			...
			if (formXpathScoreMapping.getForm().getFormId() ==Integer.parseInt( messages.getMessage("DH_FormID",null,null))) {
				...
				.getRoleFeaturePermissionSchemeModel().getAreaModel().getAreaName());
						...
						if (facilityScore.getScore() == null
								|| formXpathScoreMapping.getMaxScore() == null) {
							bubbleDataModel.setValue(0.00);
						} else {
							bubbleDataModel
									.setValue((double) Math
											.round((facilityScore.getScore() / formXpathScoreMapping
													.getMaxScore()) * 100));
						}
						// Setting color
					...
					if (facilityScore.getScore() == null
						...
						bubbleDataModel
								.setValue((double) Math.round((facilityScore
										.getScore() / formXpathScoreMapping
										.getMaxScore()) * 100));
					}
					// Setting color
					if (bubbleDataModel.getValue() >= 80) {
	...
	public Map<String, Object> fetchTreeData() {
		...
		for (FormXpathScoreMapping formXpathScoreMapping : formXpathScoreMappings) {
			...
			indMap.put(
					"name",
					formXpathScoreMapping.getShortName()!=null?formXpathScoreMapping.getShortName()
			...
			for (FormXpathScoreMapping childFormXpathScoreMapping : childFormXpathScoreMappings) {
				if (childFormXpathScoreMapping.getParentXpathId() == formXpathScoreMapping
					...
					childMap.put("name",childFormXpathScoreMapping.getShortName()!=null?childFormXpathScoreMapping.getShortName(): childFormXpathScoreMapping.getLabel());
					childMap.put("fullName", childFormXpathScoreMapping.getLabel());
					childMap.put("Id",
	...
	public Map<String, Object> forceLayoutData(Integer sectorId, Integer areaId,int timeperiodId) {
		...
		resultMap.put("percentile",parentLastVisitData[1]==null?0.0:df.format(parentLastVisitData[1]));
		resultMap.put("value",parentLastVisitData[2]==null?0:parentLastVisitData[2]);
		resultMap.put("wt", parentLastVisitData[3] == null ? 0
				: parentLastVisitData[3]);
...
//		.getDataByparentIdsInSectorIdIdAndDistrictAreaId(sectorId, areaId,timeperiodId);
		...
		for (Object[] childlastVisitData : childlastVisitDatas) {
			...
			indMap.put("name", childlastVisitData[5]==null?childlastVisitData[0]:childlastVisitData[5]);
			indMap.put("value", childlastVisitData[1] == null ? "N/A"
					:childlastVisitData[1]);
			if(childlastVisitData[4].toString().startsWith("select_one")|| childlastVisitData[4].toString().equalsIgnoreCase("integer"))
			{
				if(childlastVisitData[4].toString().startsWith("select_one"))
				{
					indMap.put("value", childlastVisitData[1] == null ? "N/A"
							:choiceMap.get(childlastVisitData[4].toString().replace("select_one", "").trim()+"_"+String.valueOf((int)Double.parseDouble(childlastVisitData[1].toString()))));
				
				}
				else
				{
					indMap.put("value", childlastVisitData[1] == null ? "N/A"
							:childlastVisitData[1].toString());
				}
			...
			for (Object[] childnodelastVisitData : lastVisitDataRepository
				...
				childMap.put("name",childnodelastVisitData[5]==null?childnodelastVisitData[0]:childnodelastVisitData[5]);
				...
				if(childnodelastVisitData[4].toString().startsWith("select_one")|| childnodelastVisitData[4].toString().equalsIgnoreCase("integer"))
				{
					if(childnodelastVisitData[4].toString().startsWith("select_one"))
					{
						childMap.put("value", childnodelastVisitData[1] == null ? "N/A"
								:choiceMap.get(childnodelastVisitData[4].toString().replace("select_one", "").trim()+"_"+String.valueOf((int)Double.parseDouble(childnodelastVisitData[1].toString()))));
					...
					{
						childMap.put("value", childnodelastVisitData[1] == null ? "N/A"
								:childnodelastVisitData[1].toString());
					}
				...
				child1Map.put("name",childnodelastVisitData1[5]==null?childnodelastVisitData1[0]:childnodelastVisitData1[5]);
				child1Map.put("value", childnodelastVisitData1[1] == null ?"N/A"
						: childnodelastVisitData1[1]);
				child1Map.put("size", "80");
				if(childnodelastVisitData1[4].toString().startsWith("select_one")|| childnodelastVisitData1[4].toString().equalsIgnoreCase("integer"))
				{
					if(childnodelastVisitData1[4].toString().startsWith("select_one"))
					{
						child1Map.put("value", childnodelastVisitData1[1] == null ? "N/A"
								:choiceMap.get(childnodelastVisitData1[4].toString().replace("select_one", "").trim()+"_"+String.valueOf((int)Double.parseDouble(childnodelastVisitData1[1].toString()))));
						
					}
					
					else
					{
						child1Map.put("value", childnodelastVisitData1[1] == null ? "N/A":childnodelastVisitData1[1].toString());
					}
